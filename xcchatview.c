...
xc_chat_view_class_init (XcChatViewClass *klass)
{
  ...
  klass->lstview = NULL;
  klass->clippy_prime = gtk_clipboard_get (GDK_SELECTION_PRIMARY);
  klass->clippy_sec = gtk_clipboard_get (GDK_SELECTION_CLIPBOARD);

  /* virtual function overrides go here */
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  gobject_class->finalize = xc_chat_view_finalize;
...
*/
  ...
  if (xccv->search_label) {
    g_string_free (xccv->search_label, TRUE);
    gtk_widget_destroy (xccv->search_widget);
    xccv->search_label = NULL;
    xccv->search_widget = NULL;
  }

  G_OBJECT_CLASS (xc_chat_view_parent_class)->finalize (object);
}
...
xc_chat_view_attach (XcChatView *xccv, struct atview *atv)
{
  ...
  if (xccv->isdown && ! xccv->idlepshdwn_id)
    ...
    if (gtk_tree_model_get_iter (GTK_TREE_MODEL (xccv->store), &iter, xccv->toprow))
      ...
      gtk_adjustment_set_value (xccv->atv->vadj, 0);
...
static void
xc_chat_view_get_top_row (XcChatView *xccv) {
	if (xccv->toprow)
		gtk_tree_path_free (xccv->toprow);
  ...
  for (r = rows; r != NULL; r = r->next)
  {
    if (gtk_tree_model_get_iter (model, &iter, r->data))
    {
      ...
      if (klass->timestamps)
      {
        if (gd)
        {
          dt_str = g_date_time_format (gd, klass->dtformat);
